...
- [EN-2+3 ✔ `0770e83`] `models/cnn_en.pt` — التدريب: epoch1 train 0.6662 / val 0.9480 (~420s)، epoch2 val **1.0000** → `[SELFTEST] train_en: PASS`. (آخر إعادة تدريب — Polish8 —: epoch1 val **0.9998** → PASS.) `--selftest-en` يشمل margin-gate. (الأرتيfacts gitignored؛ يبقى كما هو.)
...
- [Polish8 ✔] جولة نقدية ثانية: app.py — حذف ميته (`import time`, `MOD`, `col_state`)؛ تحميل خريطة الفئات العربية مرة واحدة بدل 32 مرة؛ حماية قراءة class_map_en باحتياط رمزي. engine_en.py — `train_en` يحفظ أفضل checkpoint فقط عند التحسّن (مطابقة سلوك العربي)، و`fetch_en_data` يعيد بناء dict_en/class_map_en من npz الموجود لو أُتلفا. ALL PASS + AppTest 0.
- [Polish9 ✔] **قياس أداء الكاميرا — تشخيص بالقياس لا بالافتراض** (`bench_live` بنش مؤقت + حقن في السجل):
  - **تفنيد الشك الأول:** `_get_model`/`_get_hand_detector` كلاهما **singleton مؤكد** (same-object، يُحمَّل مرة واحدة فقط) — لا إعادة تهيئة لكل فريم.
  - **المسبب الحقيقي 1 (عطل):** الكاشف مشترك بين اللغتين وVideo-mode يشترط timestamp متزايداً؛ timestamp كان نسبياً إلى t0 فأي `LivePipeline` جديد (تبديل لغة!) تتصفّر القيم → خطأ «Input timestamp must be monotonically increasing» → الحلقة تتوقف برسالة. **الإصلاح:** `_next_ts_ms()` عداد عالمي متزايد بصرامة للكاشف، وزمن محلي لسلسلة الإشارات (صمت).
  - **المسبب الحقيقي 2 (بطء):** ترميز/إرسال PNG ينفجر مع دقة الفريم: 1080p = 25.6ms/53KB مقابل JPEG80 = 8.2ms/38KB؛ كشف اليد شبه ثابت التكلفة (27→33ms عبر 640→1920) لأن MediaPipe يكبّر داخلياً. **الإصلاح:** `downscale_live(max 640)` قبل المعالجة في app.py + `st.image(..., output_format="JPEG")` → 1.6ms/7KB.
  - **الأرقام (بنش اصطناعي 640×480، 40 إطاراً):** مراحل — detect 27.3ms، crop 0.5، classify 4.1، geometry 0.14، JPEG-encode 1.6. `LivePipeline.update`: **med 27.8ms / p90 31ms / 0 أخطاء** (40 فحصاً). القيمة السابقة: مسار الخطأ ~3ms (مكسور، بلا معالجة فعلية).
  - **قياس/إعداد:** `FrameProfiler` يسجّل متوسط أزمنة كل مرحلة كل 60 إطاراً في logs/app.log بصيغة `profile[ar|en] avg_ms/frame total=..`. كاميرا هذه الآلة 640×480: فتح 947ms وقراءة أولى 686ms (إحماء كاميرا طبيعي، isOpened=True).
  - فحوص selftest جديدة (5): `live.ts.monotonic`, `live.downscale.*`, `live.profile.*` → الفحص الآن **56 فحصاً ALL PASS** + AppTest 0 استثناءات بكلتا اللغتين.
- [Polish10 ✔] **تحقق هندسي إنجليزي (SIGNPAT_EN)** — قِيس على asl_mnist.npz نفسها: رسومات MNIST منحازة منهجياً (عتب أعمدة الأصابع: B≈2 لا 4، A≈3 لا 0، W≈4 لا 3) فلا تصح أساساً صارماً؛ الملمس الموثوق الوحيد منها الفصل «أصابع مرفوعة (topY≈0–3)» عن «قبضة (topY≈6–9)». القرار: بوابات **عدد الأصابع فقط** (18 حرفاً) من القيم الكنسية لـASL بتسامح ±1 وإبهام "any"؛ الحروف المنحنية/الغامضة (C,G,O,P,Q,X) بلا قيد (margin-check وحده). `validate_geometry(patterns=)` عمّم للجدولين؛ الحرف-بحرف الصارم يؤجل إلى EN-6. الفحص الآن 64 فحصاً ALL PASS + AppTest 0.
- [Polish11 ✔] **Goal2 لمسات 2–4**: (2) سلاسة الواجهة — انتقالات CSS لشريط الثقة (width/opacity 0.12s) والباندج/الحرف + `min-height` ثابتة تمنع قفزة الارتفاع بين حالات «حرف/غير معروف/بانتظار»؛ (3) زر «⬇ تحميل آخر تسجيل صوتي (mp3)» بجوار التشغيل التلقائي، آخر صوت محفوظ في session_state لكل لغة خارج الـfragment 0.1s ليكون الزر قابل للنقر؛ (4) REPORT.md حدّث القرارات (SIGNPAT_EN، تدرّج الكاميرا+JPEG بأرقام Polish9) والحدود والمستقبلي. AppTest 0/0.
- [UI-1 ✔] **بصرية فقط – تكوين الحرف/الكلمة** (`e6cf490`): كل حرف مُلتزم يظهر كبلاطة سكرابل (fade+scale+rotate عبر `@keyframes tile-in`، 40 خانة ثابتة = `MAX_WORD_LEN` بلا إعادة إنشاء)؛ كشف AI عند إغلاق الكلمة: رقائق الخام تتلاشى ويظهر المصحح أكبر/ذهبي + شارة AI (`.reveal-stage.done`)؛ السجل فقاعات شات (تدخل بـ `bubble-in`، فقاعة AI يمين/ذهبية). HTML/CSS فقط في app.py — محركات التصنيف/التشفير/SIGNPAT لم تُلمس؛ AppTest ar/en صفر استثناءات. ملاحظة: افتراض «Streamlit يعيد كتابة innerHTML في fragment» → الأنيميشن أحادي اللقطة على فئات عابرة (مقيّد داخل الفقرة فقط).
- [UI-2 ✔] **بصرية فقط – مؤشرات أوضح** (`5b7f509`): شريط الثقة يتلوّن حسب القيمة (أحمر تحت العتبة، أصفر قريب، أخضر→سماوي فوقها مع توهج `box-shadow`) عبر `_conf_bar()`؛ موجة صوت CSS خالصة (7 أعمدة بمراحل، `animation-play-state: running` لدورة 4.5s بعد الصوت)؛ `page-fade` على `.stMainBlockContainer` يلطّف التبديل بين اللغات. AppTest صفر استثناءات؛ لا لمس للمنطق.
- [UI-3 ✔] **بصرية فقط – خلفية/هوية** (`3fd09fd`): نقش إسلامي ثماني خفيف SVG (data-URI) للعربي مقابل شبكة محايدة للإنجليزي، مع توهّجات شعاعية تحت البطاقات وحاوية شفافة؛ بطاقات القاموس تُظهر shimmer خلف الصورة + `card-in` و `loading="lazy"`. AppTest صفر استثناءات؛ لا لمس للمنطق.
- [UI-4 ✔] **مكتبات**: بدون إضافات. استوفت CSS النقاط الثلاث كاملة (`@keyframes`/`transition` خفيفة)؛ **streamlit-lottie** لم يُستخدَم لأنه يضيف اعتماداً ثقيلاً (وحزمة JS + شبكة لبعض الأنيميشنات) لقيمة هامشية على HUD حي يتحدث كل 0.1 ثانية — قرار موثّق حسب شرط المستخدم. أي مكتبة مستقبلية يتطلب مبرّراً خطياً هنا قبل الإضافة.
- [UI-5 ✔] **ترجمة كاملة إنجليزي/عربي** (`99808c1`): كل نص ظاهر عبر `tr(en, ar_)` (تبويبات، أزرار بدء/إيقاف/تحميل mp3، caption Groq، أخطاء النموذج/الكاميرا، كيف تعمل، footer)؛ اتجاه خالص: LTR كامل بالإنجليزي وراء RTL كامل بالعربي عبر `_DIRCSS` (offset/align الرؤوس كذلك). بلا مزيج لغات في أي وضع. AppTest ar:0 en:0؛ لا لمس للمنطق.
- [UI-6 ✔] **تخطيط الكاميرا** (`c9d6974`): تبويب حي بعمودين (كاميرا يسار، نتيجة+ثقة+كلمة+سجل يمين) + سقف ارتفاع الكاميرا 380px عبر `.stImage img { max-height: 380px }` — كل العناصر ظاهرة بلا سكرول رأسي على شاشة عادية. AppTest ar:0 en:0.
- [UI-7 ✔] **قاموس بصري بدل شمسية/قمرية** (`11197e1`): التصنيف الحرفي العربي (شمسي/قمرية/خاصة) حُذف نهائياً من القاموس. بديله تصنيف بصري لشكل اليد في كل صورة (أصابع مفرودة/قبضة مغلقة/إبهام بارز) وفق تصنيف المستخدم البصري في `dict_classes.tsv` الملتزم مع الكود كأصل بيانات: 25 extended + 3 fist (07,10,20) + 4 thumb (02,06,28,29). ألوان/حدود/شارات مخصصة لكل فئة (sky/orange/violet) مع تسمية ثنائية اللغة ووسم «مرجع تعليمي — ليس نتيجة كشف حي». فئة CSS `badge-qamari` أعيدت تسميتها `badge-ai` (خاصة شارة AI فقط). AppTest ar:0 en:0.
- [UI-8 ✔] **فحص «Fox» وأوضح السجل** (`59349f0`): التحقيق الشامل (grep كل الملفات + `git log -S 'fox'/'Fox'/'FOX'` بلا نتائج) أثبت ألا «Fox badge» في الكود — المصدر كلمة حيَّة مُسجَّلة F·O·X (فئات EN 5/13/22) ظهرت كفقاعة سجل. الحل: فقاعات السجل تعرض الآن كل دخول كتسلسل حروف صريح «signed: F · O · X» / «مُسجَّل: ف · ...» فيزول الغموض بين كلمة كاملة وتسلسل مُملى. AppTest ar:0 en:0.
- [UI-9 ✔] **تشخيص تصحيح** (`74ad9db`): مربع «🔧 معلومات تشخيصية» خارج الـ fragment (منطفئ افتراضياً) يعرض اللغة النشطة + اسم ملف checkpoint المحمّل فعلياً `_PCMODEL.name` (cnn.pt/cnn_en.pt — الملفان مُتحقق من وجودهما)؛ بلا أي ملف افتراضي غير موجود. AppTest ar:0 en:0 (مع تشغيل المفتاح).
- [UI-10 ✔] **كلمات شائعة** (`0c650b3`): تبويب ثالث «كلمات شائعة» ببطاقات ثابتة (بلا كاميرا/تدريب): عربي 7 كلمات (شكراً، نعم، لا، أهلاً، سلام عليكم، أنا، مساعدة) وإنجليزي 6 (Hello, Thank you, Yes, No, Please, Help) — كل بطاقة: الكلمة + الحروف مفصولة «·» + وصف قصير لشكل اليد + شارة «مرجع» ووسم صريح «مرجع تعليمي — ليس نتيجة كشف حي». AppTest ar:0 en:0.
