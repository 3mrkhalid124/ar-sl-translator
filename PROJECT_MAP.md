...
14. **تشخيص «الكاميرا بطيئة» (جلسة قياس 06-09-2026، بلا commit)**: قِيس بالبنش ms/مرحلة (copy 0.1 · crop 0.8 · classify 3.8 · detect p50≈21–31 · update p50≈21–31/p90≈30–54 — المحرك طبيعي). مدحوضة فرضية الـ timestamp القديمة (TS_MONOTONIC PASS لـ2000 استدعاء، صفر استثناءات، عدّاد مشترك AR/EN بلا تصفير عند التبديل). اكتُشف **تجميد عرضي نادر داخل `HandLandmarker.detect_for_video`**: تحميلان مؤكدان (7.95s AR في 236 إطاراً، 12.03s EN في 90 إطاراً) + وضع 213ms ممتد ×2، اثمانها حلقة حصرية >31 ألف إطاراً (بما فيها 18,310+11,650) بصفر تحميل >1.2s → عرضية غير قابلة لإعادة الإنتاج في سيطرة. أُضيفت أداة تتبع `[LIVE-VOTE]` (engine.py/engine_en.py عند ENGINE_TRACE=1) تُثبت من الـ Log أن نافذة الأغلبية (5/7) مستدعاة فعلاً من `update→feed` (لاحظ `commit=YES` عند 5/7 ثم `_wins.clear()`). تُرِك الإصلاح الجذري معطّلاً عديم الفائدة تحت الميزانية — موثّق كحد معروف في REPORT.md §5، وخط الدفاع التشغيلي: زر «✓ تثبيت» اليدوي عند أي تجميد.
...
- [o] تجميد عرضي نادر في HandLandmarker على بعض الأجهزة — موثّق كحد معروف (REPORT.md §5)؛ خط الدفاع: زر «✓ تثبيت» اليدوي
